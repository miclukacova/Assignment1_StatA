...
To illustrate that the conditional distribution of $\begin{pmatrix} X_1 \\ X_2 \end{pmatrix} \vert X_3 = x_3$ is as expected, we condition on $X_3 = 0$. We approximate this ($X_3$ has a continuous distribution), by subsetting the rows of $X$ for which $|X_3| \leq 0.05$:
...
From example 2.5 we know that the variance matrix in the conditional distribution of Gaussian random variables is independent of the value of the conditioning variable. Having illustrated that the two random variables $X_1  \vert X_3 = 0$ and $X_2 \vert X_3 = 0$ are independent and with the variance we expect, this will also be true for all other values of $X_3$. 
...
where $\beta_0$ is the global intercept corresponding to `ProductYoghurtA` and $\beta_i$ for $$i \in \{ProductYoghurtA, ProductYoghurtB, ProductYoghurtC, ProductCheeseA, ProductCheeseB$$ $, ProductCheeseC\}$ describes the difference between the global intercept and each other product. Note, that there in fact is no $\beta_{ProductYoghurtA}$ in the model as this is included in $\beta_0$. The $B_j^{P} \overset{i.i.d} \sim \mathcal{N}(0, \tau_P^2 I)$ is the random parameter of participant $j = 1,...,75$ and $B_k^{C} \overset{i.i.d}\sim \mathcal{N}(0, \tau_C^2 I)$ is the random parameter of class $k=1,...,5$. Finally, $\epsilon_{ijk} \overset{i.i.d}\sim \mathcal{N}(0, \sigma^2)$ is the residual for participant $j$ in class $k$, testing product $i$.

We model participant and class as random effects since these variables are random samples from a larger population and we are as such not interested in these specific observations, only the variance they add to the model through dependence structures. We model `Product` as a fixed effect as we want to estimate, how the different products effect the overall liking.


## 2.

The formula for the correlation for two random variables $X$ and $Y$ is $\frac{Cov(X,Y)}{\sqrt{(VY\cdot VY)}}$. By the independence of the random variables in the statistical model for `fit1` we get that $VY_{ijk}=\tau_{P}^2+\tau_{C}^2+ \sigma^2$ for all $i,j$ and $k$. We now consider the two different cases:
...
knitr::kable(data, col.names = c("Cor(Y_ijk, Y_ljk)"))
```
...
Plotting in the estimates of the standard deviations from `fit1` and `fit2` we get:
```{r, echo = FALSE, warning=FALSE}
tauP1 <- data.frame(VarCorr(fit1))$sdcor[1]
tauC1 <- data.frame(VarCorr(fit1))$sdcor[2]
sigma1 <- data.frame(VarCorr(fit1))$sdcor[3]
...
fit1_cp <- (tauP1^2)/(tauP1^2 + tauC1^2 + sigma1^2)
fit2_cp <- (tauP2^2)/(tauP2^2 + tauC2^2 + sigma2^2)
...
knitr::kable(data, col.names = c("Cor(Y_ijk, Y_lpk)"))
```
...
To test whether the children like all three versions of the product equally well, we can check that none of the fixed effects are significantly different from each other. That is, we test the two hypotheses: $H_{01}: \beta_{ProdVersion} = 0$ and $H_{02}: \beta_{ProdType} = 0$. Both of these hypotheses are tested using the `drop1()` function with `test = "Chisq"`:

```{r}
drop1(fit2, test = "Chisq") 
```

We get a p-value of `0.011` for `ProdVersion` and a p-value of `< 2e-16` for `ProdType`, so on a 5% significance level, we reject both null hypotheses. This means, there is both evidence that the children do not like all three versions equally well and that the children do not like cheese and yoghurt equally well. We should keep in mind, that the chisquare test is an asymptotic test, and we do not know if the asymptotics have set in, so we should be careful drawing conclusions, especially for `ProdVersion` as this is only borderline significant. However, based on the tests we continue to examine in what way the preferences differ.

From the model coefficients, we see evidence that the children like yoghurt better than cheese. To check which product version the children like better, we determine the 95%-profile likelihood confidence interval for the model parameters:
...
From the confidence intervals we see no strong evidence that the children like `ProductVersionB` better than `ProductVersionC`. To sum up: On a 95% significance level the children like `ProductVersionA` better than `ProductVersionC`, but there is not enough evidence that the children like `ProductVersionA` better than `ProductVersionB` or `ProductVersionB` better than `ProductVersionC`, according to this model.


## 5. 
We use the code from the hint to run 2000 simulations from `fit2`. We first extract the modelmatrices $X$ and $Z$ and the parameter estimates for $\beta, \tau_P^2, \tau_C^2$ and $\sigma^2$. We then create an empty $3\times2000$ matrix to store the simulations of $\hat{\delta}$ and the corresponding Wald confidence bands. Then we fill out the entries of the matrix with a for loop. So for each $i$ we simulate 75, 5 and 450 values from $B^P, B^C$ and $\epsilon$ respectively, and use those to create a new set of y-values $y = X\beta + Z(B^P + B^C) + \epsilon$. We then fit a new model specified as `fit2` and calculate corresponding confidence intervals using the Wald-method.
...
```{r}
mean(deltasim$lower <= fixef(fit2)[4] & fixef(fit2)[4] <= deltasim$upper) |> knitr::kable(col.names = " ")
```
...
The estimates still appear unbiased and the confidence intervals achieve accurate coverage. We can conclude that the model estimates and confidence intervals are robust to the type of distribution as long as the mean and variance are correctly specified. 
...
```{r, echo=FALSE, warning=FALSE, message=FALSE,fig.width=9, fig.height=6, fig.align='center'}
...
We may be tempted to conclude that the residuals display an alarming pattern. We should however keep in mind, that the response is discrete while the fitted values are not. Hence, the lines in the plot, are in fact to be expected. In particular, for each response value, $a = 1,...,7$, we have a linear relationship between the fitted values and the residuals $Resdiual = a - Fitted$. The line fitted with `geom_smooth` suggests, that the residuals have mean around zero, and display no concerning trend. We run a few simulations to check the if we can replicate the residual plot:
...
Note, we have discretized the response in the simulations to get a more similar picture of the residuals. We have no other restrictions on the predictions, so the model simulates responses outside the range 1 to 7. This gives more lines that the original residual plot, but other than this, the residual plots for the simulations resemble the residual plot for the original model. Hence, it seems reasonable to conclude that the model captures the mean structure of the data.
...
In order to examine how discretization of the response affects the LMM-based estimator for $\delta$, we simulate from the `fit2` model, discretize the simulations with the `round` function, fit a model on the simulated discretized data and extract the estimates and the CI boundaries. 
...
So we still obtain the desired coverage and it seems that $\hat{\delta}$ is still an unbiased estimator, regardless of the discretization of the predictor-variables.
