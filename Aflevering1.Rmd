...
The qq-plot suggests that the residuals by and large are normal. There are some issues in the upper tail suggesting that the distribution of the residuals may be slightly left-skewed, so the residuals are perhaps not perfectly normal. This is not a major concern though, as the normality assumption is not crucial for the validity of the model, as we also saw in the simulations in the previous question.

We finally inspect the predicted random effects:
