...
If we now look at the correlation between observations from the same class, but not the same participant, we have independence between all random variables in the model except for $B_{k}^{\text{class}}$. So by a similar argument to that above we get:
$$
cor(Y_{ijk},Y_{lpk})=\frac{\tau_{\text{class}}^2}{\tau_{\text{par}}^2+\tau_{\text{class}}^2+\sigma^2}
$$

*Skal vi ikke rent faktisk beregne estimaterne her ud fra de to modeller?*
